...
for hour in unique_times:
    ...
    # SVR model with non-linear kernel (e.g., RBF)
    svr = SVR(kernel='rbf')
    svr.fit(X_scaled, y_target)
    
    # Calculate feature importances using permutation importance
    perm_importance = permutation_importance(svr, X_scaled, y_target)
...
plt.title('Pattern of Feature Importances over Different Times of the Day (SVR)')
plt.legend()
plt.grid(True)
plt.savefig('./Support Vector Regression/svr_feature_importances_weekend.png')
plt.show()
plt.close()
...
for entry in actual_vs_predicted:
    ...
    plt.title(f'Actual vs Predicted Total Toll at Hour {hour} (SVR)')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'./Support Vector Regression/weekend results/{hour}.png')
    # plt.show()
    plt.close()
